...
def test_matmul_layernorm_forward():
    ...
    separate_y = ad.layernorm(ad.matmul(x1, x2), normalized_shape=[4])
...
def test_matmul_layernorm_backward():
    ...
    x2_val = torch.tensor(
        [[7.0, 8.0, 9.0, 10.0], [10.0, 11.0, 12.0, 13.0], [13.0, 14.0, 15.0, 16.0]]
    )
    ...
    expected_grads = separate_evaluator.run(
        {x1: x1_val, x2: x2_val, y_grad: y_grad_val}
    )
...
def test_matmul_softmax_forward():
    ...
    separate_y = ad.softmax(ad.matmul(x1, x2), dim=-1)
